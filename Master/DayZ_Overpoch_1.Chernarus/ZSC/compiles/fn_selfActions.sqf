...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
		...
		if(_owner == dayz_characterID || (getPlayerUID player) in _allowed)then{            
				s_player_plotManagement = player addAction ["<t color='#0059FF'>Manage Plot</t>", "zsc\plotManagement\initPlotManagement.sqf", [], 5, false];
			};
		};
			...
			if(locked _cursorTarget) then {
				if(_hasKey || _oldOwner) then {
					...
					if(_hasHotwireKit) then {
						_Unlock = player addAction [format[localize "STR_EPOCH_ACTIONS_HOTWIRE",_text], "ZSC\compiles\hotwire_veh.sqf",_cursorTarget, 2, true, true, "", ""];
					} else {
